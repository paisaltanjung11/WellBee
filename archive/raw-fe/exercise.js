...
function initWorkoutSelection() {
  ...
  // Hide all recommended tags initially
  document.querySelectorAll(".tag-recommended").forEach((tag) => {
    tag.style.display = "none";
  });

  // In a real app, this would be based on user profile and preferences
  // For now, show recommended tags based on some condition (e.g., BMI)
  // You can modify this logic based on your requirements
  const recommendedWorkouts = ["home", "outdoor", "gym"]; // Example: all workouts are recommended
  recommendedWorkouts.forEach((workout) => {
    const tag = document.getElementById(`${workout}Recommended`);
    if (tag) {
